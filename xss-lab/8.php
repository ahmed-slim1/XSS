...
<body>
    <div class="navbar">
        ...
        <a href="1.php" class="active">🟢 Level 1</a>
        ...
        <a href="8.php"  class="active">🔴 Level 8</a>
    ...
    <div class="container">
        <h1>Level 8: XSS </h1>
        <p>لاكن يبدو انه به يعض المشاكل  JSON  في هذا المستوى، لدينا تطبيق ويب يقبل إدخال بيانات بصيغة </p>
	<p>فقط JSON  فهو يقبل كل شئ وليس صيغة   </p>
        <p>سيكون الأمر سهلاً لأنه لا يوجد أي نوع من أنواع الحماية، ولكن التحدي يكمن في عدم معرفتك بمكان إدخال البيانات بدقة</p>
	 <form action="" method="POST">
        ...
        $json = $_POST['input'];
        $_SESSION['json_data'] = $json;
      }

	if (isset($_SESSION['json_data'])) {
        $json_data = $_SESSION['json_data'];
	echo '<div class="example-code">';
        echo "<script>var data = JSON.parse('$json_data'); document.write(data.message);</script>";
	echo '</div>';
        ...
        <div class="hints">
            ...
            <div id="hint1-content" class="hint-content">
                 فكر في كيفية إدخال بيانات يمكن أن تكسر هيكل الصفحة الحالي وتسمح لك بإدخال كود جديد
            </div>
            <div id="hint2-content" class="hint-content">
             <pre>&lt;?php
        echo "&lt;script&gt;var data = JSON.parse('$json_data'); document.write(data.message);&lt;/script&gt;";
  ?&gt;</pre>
             $json_data	هذا هو الكود الذي الكود انت تكتب بداخل
            </div>
            <div id="hint3-content" class="hint-content">
                        كل ما عليك فعله هو الخروج من كل هذا وتنفيز ما تريد <br>
			<pre>&lt;/script&gt;&lt;script&gt;alert('XSS')&lt;/script&gt;</pre>
            </div>
        </div>
        <footer>
                          Ahmed Mahmoud Selim
