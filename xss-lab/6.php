...
<body>
	...
	<a href="index.php">🏠 Main</a>
        ...
        <a href="6.php" class="active">🟠 Level 6</a>
    ...
    <div class="container">
        <h1>Level 6: XSS</h1>
     	 <p>  JavaScript  يبدو أن مطور الموقع قد أضاف بعض الحمايةلكن يبدو أن لديه معرفة محدودة بـ   </p>
        <form method="GET">
        ...
        if (isset($_GET['input'])) {
            $input = $_GET['input'];
           $finder = preg_match_all('#(src|var|eval|onmouseover|onload|onclick|onmouseout|>|<|onerror)#i', $input);
	   if($finder){
   	   echo '<div class="example-code">';
           ...
           echo '</div>';
            ...
            echo '<div class="example-code">';
            echo '<img src=imag.jpeg value="'.$input.'" >';
            echo '</div>';
        }
	}
        ?>
        <div class="hints">
            ...
            التي قد لا يعرفها المطور Event Handlers  لذا حاول تجربة بعض ال Event Handlers يبدو أن الموقع يقوم بحظر يعض ال 
            </div>
            <div id="hint2-content" class="hint-content">
                  JavaScript Event Handlers  يمكنك البحث في جوجل عن 
		                                          <br><br>
                ملاحظة: يقوم الموقع بحظر الأوامر التالية فقط <br><br>  `onfocus`, `var`, `eval`, `onmouseover`, `onload`, `onclick`, `onmouseout`, `script`, `onerror`
	   </div>
            <div id="hint3-content" class="hint-content">
                        : مثل Event Handlers يمكنك استخداد <br><br> "onmouseup=alert(1) "   <br><br>   كاملاً، ثم اضغط على الصورة ليتم تنفيذ الأمر HTML لا تنسَ أن تكتب علامتي " " لكي يكون كود الـ  
            </div>
        </div>
        <footer>
            Ahmed Mahmoud Selim
